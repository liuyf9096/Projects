...
SlotC2::SlotC2(int pos, const QString &group, QObject *parent)
    : SlotBase{pos, group, parent}
{
    ...
    m_isAutoSolutionFill = FCommon::GetInstance()->getConfigValue("stain", "auto_fill_solution", "auto_fill_c2").toBool();
...
void SlotC2::setMaxStainCount(int stainCount)
{
...
void SlotC2::setSolutionExpiryTime(int min)
{
...
bool SlotC2::drainSlot(quint64 id)
{
...
bool SlotC2::cleanSlot(Detergent d, quint64 id)
{
...
// 1s
void SlotC2::onExpiryTimeout_slot()
{
...
void SlotC2::prepareSolution(const QString &sid)
{
    if (s_state == Process::Idle) {
        m_prepare_slideid = sid;
        m_processTimer->start();
        qDebug() << QString("%1 prepareSolution, sid:%2").arg(mSlotInfo).arg(sid);  
    }
}

void SlotC2::handlePutinSlide()
{
    /* record */
    Q_ASSERT(m_slide);
    m_slide->sql_recordStainStatus("c2_0", QTime::currentTime().toString("HH:mm:ss"));
}

void SlotC2::handleTakeoutSlide()
{
    ...
    /* record */
    Q_ASSERT(m_slide);
    m_slide->sql_recordStainStatus("c2_1", QTime::currentTime().toString("HH:mm:ss"));
}


void SlotC2::onSolutionProcess_slot()
{
    switch (s_state) {
    case Process::Idle:
    ...
    case Process::Fill_Solution:
        mInfuser->addRequest("a1b1", mPos, pa, pb);
...
void SlotC2::onDrainProcess_slot()
{
    switch (s_drain) {
