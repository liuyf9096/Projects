...
static const QString AppName = "SmearMain";
...
FCommon::FCommon(QObject *parent) : QObject(parent)
{
    ...
    m_verbose = false;
    m_isUnited = getConfigValue("united", "enable").toBool();
    m_isTrackEnable = getConfigValue("track", "enable").toBool();
    m_isPrintEnable = getConfigValue("print", "enable").toBool();
    m_isReaderEnable = getConfigValue("reader", "enable").toBool();
}

void FCommon::printConfigInfo()
{
    qDebug() << "Config Content:" << m_configObj << endl;
    qDebug() << "Set United:" << m_isUnited;
    qDebug() << "Set TrackEnable:" << m_isTrackEnable;
    qDebug() << "Set PrintEnable:" << m_isPrintEnable;
    qDebug() << "Set ReaderEnable:" << m_isReaderEnable;
    if (m_isUnited && m_isTrackEnable) {
        qInfo() << "!!! Setting Error.";
    }
    qDebug() << " ";
}
